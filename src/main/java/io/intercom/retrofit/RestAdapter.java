...
import io.intercom.retrofit.Profiler;
import io.intercom.retrofit.RxSupport;
...
import io.intercom.retrofit.converter.ConversionException;
import io.intercom.retrofit.converter.Converter;
import io.intercom.retrofit.mime.MimeUtil;
    ...
    private class RestHandler implements InvocationHandler {
            ...
            if (methodInfo.isSynchronous) {
                ...
                if (RestAdapter.this.rxSupport == null) {
                    if (Platform.HAS_RX_JAVA) {
                        RxSupport unused = RestAdapter.this.rxSupport = new RxSupport(RestAdapter.this.httpExecutor, RestAdapter.this.errorHandler, RestAdapter.this.requestInterceptor);
                    } else {
                ...
                RestAdapter.this.httpExecutor.execute(new CallbackRunnable(args[args.length - 1], RestAdapter.this.callbackExecutor, RestAdapter.this.errorHandler) {
        ...
        public Object invokeRequest(RequestInterceptor requestInterceptor, RestMethodInfo methodInfo, Object[] args) {
            ...
            try {
                ...
                url = request.getUrl();
                if (!methodInfo.isSynchronous) {
                    int substrEnd = url.indexOf(Condition.Operation.EMPTY_PARAM, serverUrl.length());
                    if (substrEnd == -1) {
                ...
                if (body != null) {
                    ...
                    if (!methodInfo.isSynchronous) {
                        Object convert2 = new ResponseWrapper(response, convert);
                        if (!methodInfo.isSynchronous) {
                            Thread.currentThread().setName(RestAdapter.IDLE_THREAD_NAME);
                        }
                        return convert2;
                    ...
                    ResponseWrapper responseWrapper2 = new ResponseWrapper(response, (Object) null);
                    if (methodInfo.isSynchronous) {
                ...
                throw RetrofitError.conversionError(url, Utils.replaceResponseBody(response, (TypedInput) null), RestAdapter.this.converter, type, e);
        ...
        if (this.logLevel.ordinal() >= LogLevel.HEADERS.ordinal()) {
            ...
            if (body != null) {
                ...
                if (this.logLevel.ordinal() >= LogLevel.FULL.ordinal()) {
                    ...
                    this.log.log(new String(bodyBytes, MimeUtil.parseCharset(body.mimeType(), HttpRequest.CHARSET_UTF8)));
                }
            }
