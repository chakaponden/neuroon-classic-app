...
class AnswersRetryFilesSender implements FilesSender {
    ...
    public static AnswersRetryFilesSender build(SessionAnalyticsFilesSender filesSender2) {
        return new AnswersRetryFilesSender(filesSender2, new RetryManager(new RetryState(new RandomBackoff(new ExponentialBackoff(1000, 8), JITTER_PERCENT), new DefaultRetryPolicy(5))));
    }

    AnswersRetryFilesSender(SessionAnalyticsFilesSender filesSender2, RetryManager retryManager2) {
        this.filesSender = filesSender2;
